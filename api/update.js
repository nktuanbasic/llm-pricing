module.exports = async function handler(req, res) {
    ...
    if (!apiKey) {
        return res.status(400).json({ error: 'Server missing GEMINI_API_KEY env var' });
    }
...
Return ONLY valid JSON array. No markdown. Use 2 decimal prices. Mark best-value models with BEST in last element.`;
    ...
    try {
        ...
        if (!Array.isArray(jsonData)) {
            return res.status(502).json({ error: 'Gemini returned invalid format', raw: text.substring(0, 200) });
        }
